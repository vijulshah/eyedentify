...
from train_utils import print_on_rank_zero, seed_everything
...
def prepare_dataloader(dataset_configs, dataloader_configs, seed_val):
    """
    ...
    dataset_paths, output_folder = DATASET_PATHS_REGISTRY.get(
        dataset_configs["dataset_paths_registry"]
    ...
    if (
        ...
        train_dataloader = torch.load(
            f"{output_folder}/train_dataloader.pth", map_location=map_location
        )
        val_dataloader = torch.load(
            f"{output_folder}/val_dataloader.pth", map_location=map_location
        )
        test_dataloader = torch.load(
            f"{output_folder}/test_dataloader.pth", map_location=map_location
        )
        train_dataset = torch.load(
            f"{output_folder}/train_dataset.pth", map_location=map_location
        )
        val_dataset = torch.load(
            f"{output_folder}/val_dataset.pth", map_location=map_location
        )
        test_dataset = torch.load(
            f"{output_folder}/test_dataset.pth", map_location=map_location
        )
    else:
        ...
        if strategy == "cross_validation":
            ...
            print_on_rank_zero(
                "left_out_participants_for_val = ", left_out_participants_for_val
            )
            left_out_participants_for_test = dataset_configs.get(
                "left_out_participants_for_test", unique_participants_list[-2]
            )
            print_on_rank_zero(
                "left_out_participants_for_test = ", left_out_participants_for_test
            )
            train_dataset_paths = [
                item
            ...
            val_dataset_paths = [
                item
                for item in dataset_paths
                if int(item[0]) in left_out_participants_for_val
            ]
            test_dataset_paths = [
                item
                for item in dataset_paths
                if int(item[0]) in left_out_participants_for_test
            ]
        ...
        unique_participants_list_in_train = list(
            set(sorted([int(item[0]) for item in train_dataset_paths]))
        )
        print_on_rank_zero(
            "unique_participants_list_in_train = ", unique_participants_list_in_train
        )
        print_on_rank_zero("train_dataset_paths length = ", len(train_dataset_paths))

        unique_participants_list_in_val = list(
            set(sorted([int(item[0]) for item in val_dataset_paths]))
        )
        print_on_rank_zero(
            "unique_participants_list_in_val = ", unique_participants_list_in_val
        )
        print_on_rank_zero("val_dataset_paths length = ", len(val_dataset_paths))

        unique_participants_list_in_test = list(
            set(sorted([int(item[0]) for item in test_dataset_paths]))
        )
        print_on_rank_zero(
            "unique_participants_list_in_test = ", unique_participants_list_in_test
        )
        print_on_rank_zero("test_dataset_paths length = ", len(test_dataset_paths))

        if dataset_configs.get("normalize", False):
            if os.path.exists(f"{output_folder}/train_mean.pth") and os.path.exists(
                f"{output_folder}/train_stds.pth"
            ):
        ...
        train_sampler = (
            DistributedSampler(train_dataset) if torch.cuda.is_available() else None
        )
