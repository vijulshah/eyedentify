...
class ExtractorDlib:
    ...
    def __init__(self, upscale=1):
        ...
        self.face_landmarks_predictor = dlib.shape_predictor(
            f"{ROOT_DIR}/data/shape_predictor_68_face_landmarks.dat"
        )
    ...
    def extract_iris(self, image):
        ...
        cropped_left_iris = self.center_crop(
            eyes_data["left_eye"], 16 * self.upscale, 16 * self.upscale
        )
        cropped_right_iris = self.center_crop(
            eyes_data["right_eye"], 16 * self.upscale, 16 * self.upscale
        )
